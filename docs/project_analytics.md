...
| `get_grade_breakdown_by_store_comparison(from_batch_id, to_batch_id)` (added 2026-09-01, replaces the old single-batch drill-down) | List of `{store, counts: [{grade, from, to}, ...], total_from, total_to}` — unions the store lists from both batches' `grade_counts['by_store']`. Powers the "by Registration Store" From/To matrix table (green text when `to > from`, red when `to < from`). Same all-`'No Grade'`-store exclusion as `get_grade_breakdown_by_store()`. |
...
| `get_grade_changes(from_batch_id, to_batch_id, store=None, grade=None, direction=None, limit=20, offset=0)` (added 2026-09-01) | `(rows, total_count)` — the "Comparison / Members Who Changed Grade" (movers) feature. Reads `grade_members` for exactly the 2 given batches (never more), builds `{vip_id: grade}` maps for each side, diffs via set intersection of `.keys()`, excludes VIP ID `'0'` and any grade outside `DISPLAY_GRADES`. `direction` is derived from `GRADE_ORDER` (`'upgrade'`/`'downgrade'`), not stored. Sorted by `vip_id`, sliced `[offset:offset+limit]` for pagination — `total_count` reflects the full diff before slicing. Each row is joined live against `Customer` (by `vip_id`) for `name`/`phone`/`registration_store` display fields — `None` if the customer no longer exists in the live table. |
