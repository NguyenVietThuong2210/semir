...
| `/membership/partial/store-breakdown/` | `membership_store_breakdown_partial` | AJAX partial, requires `membership.view`, rewritten 2026-09-01 — always renders the full From/To matrix for all stores (`get_grade_breakdown_by_store_comparison(from_batch_id, to_batch_id)`); the old single-store `store=` drill-down param was removed (superseded by the new movers section below) |
