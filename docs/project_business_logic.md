...
**Grade Change Date (added 2026-09-02):** the Customer Tier Progress table can additionally show, per customer, the simulated DATE of their most recent grade change (upgrade/downgrade), their next scheduled downgrade review date ("Next Review", `next_check_date`), and how many more purchases they need before that review to avoid a downgrade at their current grade. Since neither the POS export nor `Customer` stores a real grade-change date, this is reconstructed via a validated exact-date event simulation (`App/analytics/grade_simulation.py::simulate_one_customer()`, A/B-validated 98.5% agreement with live `Customer.vip_grade`, 90.5% exact-day match against real snapshot pairs) run per-customer from their first transaction through today. Because the simulation loads the entire `SalesTransaction` table, it does **not** run on every page load — it's a manual, on-demand full-DB recompute ("Calculate Grade Change Dates" button, `POST /membership/compute-grade-progress/`, permission `membership.compute`, run as an async `upload_jobs.py` job of type `grade_progress_calc`, see `App/services/grade_progress_calc.py::compute_all_grade_progress()`) that persists one `CustomerGradeProgress` row per `vip_id` (see `docs/project_models.md`). Each customer is classified `ok` (simulated final grade matches their real live grade — date is trustworthy and shown), `mismatch` (has transaction history but the simulation's final grade disagrees with the real grade — the date is stored for debugging but **the analytics layer (`get_live_customer_tier_table()`) deliberately hides `last_grade_change_date`/`change_direction`/`next_check_date` from the UI for these rows**, since an untrustworthy final state implies an untrustworthy date), or `no_data` (zero transactions to simulate from). A `vip_id` with no `CustomerGradeProgress` row at all — the compute job has simply never been run — is surfaced as `grade_progress_status='not_computed'`, distinct from `no_data`.

`next_check_date` is NOT `last_grade_change_date + 365` — the downgrade check recurs every 365 days from the last CHECK (pass or fail), not the last actual grade CHANGE, so a customer who has passed one or more consecutive annual checks without changing grade again has their true next review date further out than a naive +365 calculation. `purchases_needed_to_avoid_downgrade` is computed against this TRUE window (`[next_check_date - 365, next_check_date]`), not a naive "365 days ending today" window — an earlier version of this feature used the naive window and was corrected same-day after code review flagged it could show a materially wrong number for up to ~364 days after any grade change.

The table's `sort=` param accepts `<field>_asc`/`<field>_desc` for every column: `vip_id`, `grade`, `annual_spend`, `annual_purchase_count`, `points`, `amount_to_next_tier`, `last_grade_change_date`, `next_check_date`, `purchases_needed_to_avoid_downgrade` (non-applicable rows always sort last, regardless of direction). The Excel export (`section=tier`) includes `Points` (previously missing despite being an on-screen column — filled 2026-09-02) plus `Last Grade Change`, `Direction`, `Next Review Date`, `Purchases Needed to Avoid Downgrade` (`—` for any non-`ok` row).
...
30 permissions in `PERMISSION_DEFS`, named `{domain}.{action}` and grouped into 10 domains:
...
| `membership.compute` | Compute Grade Change Dates | Customer Membership |
