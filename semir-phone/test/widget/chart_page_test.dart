...
Widget buildSubject(AsyncValue<ChartPayload?> state) {
  return ProviderScope(
    overrides: [
      salesChartProvider.overrideWith(() => _FakeNotifier(state)),
    ],
    child: MaterialApp(
      theme: buildAppTheme(),
...
void main() {
  ...
  testWidgets('data state: correct donut card count rendered', (tester) async {
    await tester.pumpWidget(buildSubject(AsyncValue.data(_twoDonutPayload())));
    await tester.pumpAndSettle();
  ...
  testWidgets('null trend → no trend section in widget tree', (tester) async {
    await tester.pumpWidget(
        buildSubject(AsyncValue.data(_twoDonutPayload(withTrend: false))));
    await tester.pumpAndSettle();
  ...
  testWidgets('trend present → "Trend" section header shown', (tester) async {
    await tester.pumpWidget(
        buildSubject(AsyncValue.data(_twoDonutPayload(withTrend: true))));
    await tester.pumpAndSettle();

    // Scroll down to bring the trend section into the viewport (it may be
    // below the fold in the lazy ListView).
